Public Class Class_AddDesserts

    Dim util As New Class_Utils
    Dim htCounter As Integer = 0
    Dim buttonCount As Integer = 0
    Dim currentCatagory As String = "Desserts"
    Public buttonDataLinkDesserts As Hashtable = New Hashtable
    ...
    Public Sub init()
        ...
        htCounter = Nothing
        buttonCount = Nothing
        buttonDataLinkDesserts = New Hashtable

        'check if a menu file exists, if not create it
        util.checkAndCreateMenuFile(currentCatagory)

        'For every line in the text file update a placeholder button with the properties specified in the text file.
        For Each line As String In System.IO.File.ReadLines(util.path & "\" & currentCatagory & ".menu")
            ...
            'Counter used for hashtable
            htCounter += 1
            buttonDataLinkDesserts.Add(htCounter, values(1)) 'Add the second part of the line to a hashtable, This is used as a data link to link to the inventory database and get the price and other information.
